...
class Patients extends BaseController
{
    ...
    public function filter()
    {
        ...
        if ($field && $value) {
            if ($field === 'full_name') {
                ...
                $patients = $model->like($field, $value)->findAll();
            } else {
                $patients = $model->like($field, $value)->findAll();
            }
        } else {
    ...
    public function save()
    {
        ...
        // Define the new file upload path in the public directory
        // C:\xampp\htdocs\hms_ram\public\uploads\patient_reports\
        $uploadDir = ROOTPATH . 'public/uploads/patient_reports/'; // <-- Corrected path
        ...
        // --- IMPORTANT: Adjusting validation for multiple files ---
        // For multiple files, you validate each file in the collection.
        // The 'uploaded' rule checks if *any* file was uploaded for the input name.
        // 'max_size' and 'ext_in' rules are applied to each file.
        // If you want to make the file upload optional, you can remove 'uploaded[upload_reports]'.
        $fileValidationRules = [
            'upload_reports.*' => [ // This wildcard applies rules to each file in the array
        ...
        // If it's a new registration and files are *required*, you might add this:
        // if (empty($this->request->getPost('id')) && !($this->request->getFiles('upload_reports')['upload_reports'][0]->isValid() ?? false)) {
        //     $fileValidationRules['upload_reports'] = [
        //         'rules' => 'uploaded[upload_reports]',
        //         'errors' => ['uploaded' => 'Please upload at least one report file.'],
        //     ];
        // }


        $validationRules = array_merge($this->patientModel->validationRules, $fileValidationRules);

        if (!$this->validate($validationRules)) {
            $session->setFlashdata('error', 'Please correct the errors in the form.');
            // Add a check for file upload specific errors
            $fileErrors = $this->validator->getErrors('upload_reports.*'); // Use .* for multi-file errors
            if (!empty($fileErrors)) {
                $session->setFlashdata('file_errors', $fileErrors); // Separate flashdata for file errors
            }
        ...
        // If editing, retrieve existing report filenames from the database
        ...
        if (!empty($patientId)) {
            ...
            if ($existingPatient && !empty($existingPatient['reports'])) {
                ...
                if (!is_array($currentReportFilenames)) {
                    $currentReportFilenames = []; // Ensure it's an array if decoding fails
                }
            }
        }

        // Handle new file uploads
        $files = $this->request->getFiles(); // Use getFiles() to get all uploaded files

        if (isset($files['upload_reports'])) { // Check if 'upload_reports' field exists in uploaded files
        ...
        $data = [
            ...
            'reports'               => !empty($currentReportFilenames) ? json_encode($currentReportFilenames) : null, // Store JSON of filenames
            'fee'                   => $fee,
    ...
    {
        // First, check the new public upload location
        $publicFilePath = ROOTPATH . 'public/uploads/patient_reports/' . $filename;

        // Fallback to the old WRITEPATH location for legacy files (if any)
    ...
    public function edit($id = null)
    {
        ...
        $data['patient'] = $patient; // Pass existing patient data to the view
        $data['doctors'] = $this->doctorModel->findAllDoctors();
        $data['referred_persons'] = $this->referredPersonModel->findAll();

        return view('patients/register_patient', $data); // Reuse the registration view for editing
    }
    ...
    public function deleteReportFile()
    {
        if ($this->request->isAJAX()) {
            ...
            $patientModel = new \App\Models\PatientModel(); // Change if your model name is different
            ...
            // Remove file from the array and update DB
